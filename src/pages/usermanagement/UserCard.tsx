...
interface AddNewUserCardProps {
    onClose: () => void; // Function to call when the modal should close
    onUserAdded: (newUser: { fullName: string; email: string; accountType: string; status: string }) => void;
}
...
const AddNewUserCard: React.FC<AddNewUserCardProps> = ({ onClose, onUserAdded }) => {
    ...
    const handleSelectChange = (value: string, name: keyof UserForm) => {
        setFormData({ ...formData, [name]: value });
        setFormErrors({ ...formErrors, [name]: undefined });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            onUserAdded(newUserResponseData); // Inform parent about new user
            // onClose(); // <--- Modal is closed by parent after onUserAdded
    ...
    return (
        ...
        <Card className="w-full max-w-2xl mx-auto z-50">
            <CardHeader>
                ...
                <CardDescription>
                    Create a new user by providing the required information.
                </CardDescription>
            </CardHeader>
            <CardContent>
